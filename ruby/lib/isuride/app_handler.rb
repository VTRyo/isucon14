...
module Isuride
  class AppHandler < BaseHandler
    ...
    get '/notification' do
      ...
      yet_sent_ride_status = db.xquery('SELECT * FROM ride_statuses WHERE ride_id = ? AND app_sent_at IS NULL ORDER BY created_at ASC LIMIT 1', ride.fetch(:id)).first
      status =
      ...
      unless yet_sent_ride_status.nil?
        db.xquery('UPDATE ride_statuses SET app_sent_at = CURRENT_TIMESTAMP(6) WHERE id = ?', yet_sent_ride_status.fetch(:id))
      end
